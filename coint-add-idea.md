...
4. **Coin redeem**: Product e admin flag korte parbe `is_coin_redeemable = true` r set korbe `coin_price` (koto coin lagbe oi product kinte). Customer "Coin Shop" page theke coin diye order place korte parbe — টাকা lagbe na (order total = 0, payment_status = paid, method = coins).
